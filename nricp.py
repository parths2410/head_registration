...
def nonrigidIcp(sourcemesh_,targetmesh_):
    sourcemesh = copy.deepcopy(sourcemesh_)
    targetmesh = copy.deepcopy(targetmesh_)

    transformed_sourcemesh = copy.deepcopy(sourcemesh)
    
    #obtain vertices
    target_vertices = np.array(targetmesh.vertices)
    source_vertices = np.array(sourcemesh.vertices)
    #num of source mesh vertices 
    n_source_verts = source_vertices.shape[0]
    
    #normals again for refined source mesh and target mesh
    source_mesh_normals = np.array(sourcemesh.vertex_normals)
    ...
    if Debug:
        ...
        sourcemesh.paint_uniform_color([0.1,0.1,0.9])
        o3d.visualization.draw_geometries([targetmesh,sourcemesh])
    ...
    transformed_sourcemesh.vertices = o3d.utility.Vector3dVector(vertsTransformed)
